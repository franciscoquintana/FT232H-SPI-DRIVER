...
struct mpsse_context *OpenIndex(struct usb_device* usb_dev, struct usb_interface* usb_if, enum modes mode, int freq, int endianess, int interface)
{
	...
	mpsse = kmalloc(sizeof(struct mpsse_context), GFP_KERNEL);
	if(mpsse)
	{
		...
		if(ftdi_init(&mpsse->ftdi) == 0)
		{
			...
			if(ftdi_usb_open_dev(&mpsse->ftdi, usb_dev, usb_if) == 0)
			{
				mpsse->mode = mode;
...
printk(KERN_ALERT "CLOCK OK");
        ...
        printk(KERN_ALERT "MPSSE%i", mpsse->open);
	return mpsse;
}

/* 
 * Closes the device, deinitializes libftdi, and kfrees the MPSSE context pointer.
 *
...
char Version(void)
{
	...
	if(version_string)
	{
		...
		if(decimal_ptr && strlen(decimal_ptr) > 1)
		{
			...
			kstrtoint(decimal_ptr+1, 0, &minor);
		}

		kstrtoint(version_string, 0, &major);
