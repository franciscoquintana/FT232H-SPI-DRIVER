...
int fast_build_block_buffer(struct mpsse_context *mpsse, uint8_t cmd, unsigned char *data, int size, int *buf_size)
{
	...
	*buf_size = 0;
	...
	mpsse->fast_rw_buf[i++] = cmd;
	mpsse->fast_rw_buf[i++] = (rsize & 0xFF);
	mpsse->fast_rw_buf[i++] = ((rsize >> 8) & 0xFF);

	/* On a write, copy the data to transmit after the command */
	if((cmd == mpsse->tx || cmd == mpsse->txrx) && (i + size) <= (SPI_RW_SIZE + CMD_SIZE))
	{
		memcpy(mpsse->fast_rw_buf+i, data, size);
...
int FastWrite(struct mpsse_context *mpsse, char *data, int size)
{
	...
	mpsse, mpsse->fast_rw_buf = fast_build_block_buffer(mpsse, mpsse->tx, (unsigned char *) (data + n), txsize, &buf_size);
				if(mpsse->fast_rw_buf)
				{	
					if(raw_write_secure(mpsse, mpsse->fast_rw_buf, buf_size) == MPSSE_OK)
					{
						n += txsize;
					}
					else
	...
	if(is_valid_context(mpsse))
	{
		if(mpsse->mode)
		{
			while(n < size)
			{
				...
				{
					if(raw_write(mpsse, mpsse->fast_rw_buf, data_size) == MPSSE_OK)
					{
	...
	if(is_valid_context(mpsse))
	{
		if(mpsse->mode >= SPI0 && mpsse->mode <= SPI3)
		{
			while(n < size)
			{
				...
				{
					if(raw_write_secure(mpsse, mpsse->fast_rw_buf, data_size) == MPSSE_OK)
					{
